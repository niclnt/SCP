...
QPushButton#Link:hover { color: #0056b3; }
...
class TokenDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Aplicar estilos básicos y colores de la nueva paleta
        self.setStyleSheet("""
                ...
                try:
                    ...
                    if msg['type'] == 'REGISTER':
                        ...
                        if msg.get('screenshot'):
                            ...
                            if viol:
                                self.update_callback(hostname, ip, f"🔴 {viol}")
                            else:
...
class TeacherDashboard(QMainWindow):
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, 'r') as f:
                    self.ngrok_token = json.load(f).get("ngrok_token", "")
            except:
                pass
    ...
    def send_pdf_broadcast(self):
        ...
        if ok:
            QMessageBox.information(self, "OK", "Enviado")
        else:
    ...
    def apply_theme(self):
        ...
        for r in range(self.table.rowCount()):
            self.update_row_color(r, self.table.item(r, 2).text() if self.table.item(r, 2) else "")
    ...
    def update_row_color(self, row, status):
        c = {"ok": "#28a745", "alert": "#dc3545", "disc": "#6c7086"} if self.is_dark_mode else \
            {"ok": "#28a745", "alert": "#dc3545", "disc": "#9ca0b0"}
        color = c["alert"] if "ALERT" in status or "DETECTÓ" in status else c["disc"] if "Desconectado" in status else \
        c["ok"]
